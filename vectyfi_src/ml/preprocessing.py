...
def build_preprocessor(X: pd.DataFrame) -> ColumnTransformer:
    ...
    pt = [c for c in PASS_THROUGH_FEATURES if c in X.columns]
    ohe = [c for c in OHE_FEATURES if c in X.columns]
    tgt = [c for c in TARGET_FEATURES if c in X.columns]

    # The ColumnTransformer applies the correct encoding rules to the correct columns simultaneously
    preprocessor = ColumnTransformer(
        transformers=[
            ('num_bin', 'passthrough', pt),
            ('ohe', OneHotEncoder(handle_unknown='ignore', sparse_output=False), ohe),
            ('target', TargetEncoder(), tgt),
        ],
        remainder='drop',
    )
    return preprocessor
